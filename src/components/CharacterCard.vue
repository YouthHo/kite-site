...
<template>
  ...
  >
    <img :src="character.image" :alt="character.name" loading="lazy" class="w-9 h-9 object-cover rounded-full border border-[#2a2520] group-hover:border-[#9d2235]/60 transition-colors" />
  ...
  <div v-else class="text-center group cursor-pointer w-16" @click="emit('select', character)">
    <div class="relative mx-auto w-14 h-14 rounded-full overflow-hidden border-2 border-[#2a2520] group-hover:border-[#9d2235] group-hover:scale-110 transition-all duration-300">
      <img :src="character.image" :alt="character.name" loading="lazy" class="w-full h-full object-cover" />
    </div>
